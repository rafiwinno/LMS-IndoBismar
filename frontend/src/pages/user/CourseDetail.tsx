...
export default function CourseDetail() {
  ...
  if (!kursus) return (
    <div className="text-center text-gray-500 dark:text-gray-400 py-12">Kursus tidak ditemukan.</div>
  );
  ...
  const renderEmbed = (item: Materi) => {
    if (item.tipe_materi === 'video') {
      ...
      return (
        <a href={item.file_materi} target="_blank" rel="noopener noreferrer"
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-[#161b27] rounded-2xl shadow-sm border border-gray-200 dark:border-white/8 overflow-hidden">
            <div className="p-4 border-b border-gray-100 dark:border-white/8">
              <h2 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wide">Materi Pembelajaran</h2>
            </div>
            <div className="divide-y divide-gray-100 dark:divide-white/8">
              {Object.entries(grouped).map(([bab, items]) => (
                <div key={bab}>
                  ...
                  >
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{bab}</span>
                    {openBab[bab]
                  ...
                  {openBab[bab] && items.map(item => (
                    <button key={item.id_materi} onClick={() => setActiveMateri(item)}
          ...
          <div className="bg-white dark:bg-[#161b27] rounded-2xl shadow-sm border border-gray-200 dark:border-white/8 overflow-hidden min-h-[400px]">
            {!activeMateri ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-4 p-8 text-center">
                ...
                <div>
                  <p className="font-semibold text-gray-700 dark:text-gray-300 mb-1">Pilih materi untuk mulai belajar</p>
                  <p className="text-sm text-gray-400 dark:text-gray-500">Klik salah satu materi di sebelah kiri</p>
                </div>
              </div>
            ) : (
              <div className="p-6 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-medium text-gray-400 dark:text-gray-500 mb-1">{activeMateri.sub_bab}</p>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white">{activeMateri.judul_materi}</h3>
                  </div>
      ...
      <div className="bg-white dark:bg-[#161b27] rounded-2xl shadow-sm border border-gray-200 dark:border-white/8 overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-white/8">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Award size={20} className="text-red-500" /> Kuis Terkait
          </h2>
        </div>
        {kuisList.length === 0 ? (
          <div className="p-8 text-center text-gray-500 dark:text-gray-400">Belum ada kuis untuk kursus ini.</div>
        ) : (
      ...
      <div className="bg-white dark:bg-[#161b27] rounded-2xl shadow-sm border border-gray-200 dark:border-white/8 overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-white/8">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <ClipboardList size={20} className="text-red-500" /> Tugas
          </h2>
        </div>
        {tugasList.length === 0 ? (
          <div className="p-8 text-center text-gray-500 dark:text-gray-400">Belum ada tugas untuk kursus ini.</div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-white/8">
            {tugasList.map(tugas => {
                ...
                <div key={tugas.id_tugas} className="p-6 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {tugas.deskripsi && <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{tugas.deskripsi}</p>}
                    </div>
                    {sudah ? (
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500">
                    ...
                    {tugas.file_soal && (
                      <a href={`${import.meta.env.VITE_API_URL?.replace('/api', '')}/storage/${tugas.file_soal}`}
                  ...
                  {sudah && tugas.nilai !== null && (
                    <div className="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg">
                      ...
                      <div>
                        <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">Nilai: {tugas.nilai} / {tugas.nilai_maksimal}</span>
                        {tugas.feedback && <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-0.5">{tugas.feedback}</p>}
                      </div>
                    </div>
                  )}
                  {sudah && tugas.nilai === null && (
