...
export function Courses() {
  ...
  const toggleStatus = async (k: Kursus, e: React.MouseEvent) => {
    e.stopPropagation();
    const newStatus = k.status === 'aktif' ? 'draft' : 'aktif';
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${selectedCourse.status === 'aktif' ? 'bg-green-100 dark:bg-green-500/20 text-green-800 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'}`}>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-white/5">
                {kursus.map(k => (
                  <tr key={k.id} onClick={() => setSelectedCourse(k)} className="hover:bg-gray-50 dark:hover:bg-white/3 transition-colors cursor-pointer">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={e => toggleStatus(k, e)} className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium cursor-pointer capitalize ${k.status === 'aktif' ? 'bg-green-100 dark:bg-green-500/20 text-green-800 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'}`}>
                        {k.status}
                      </button>
                    </td>
